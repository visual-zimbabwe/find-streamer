...
def scrape_with_selenium():
    ...
    output_file = os.path.join(script_location, "TV_Output.xlsx")

    # LOOP 1 to 242
    for i in range(1, 243):
        page_id = f"in{i:07}"
        url = f"https://www.imdb.com/chart/tv/{page_id}/"
        ...
        try:
            ...
            if header_tag:
                ...
                clean = raw.replace("IMDb Charts", "").replace("Most Popular", "").replace("TV shows", "").strip()
                category = clean.strip(": ")
            print(f"  -> Category: {category}")

            # Get Items
            items = soup.select("li.ipc-metadata-list-summary-item")
            print(f"  -> Found {len(items)} tv shows.")
    ...
    try:
        with pd.ExcelWriter(output_file, engine='xlsxwriter') as writer:
            df.to_excel(writer, sheet_name='TVData', index=False)
            workbook = writer.book
            worksheet = writer.sheets['TVData']
        ...
        print(f"SUCCESS: Saved {len(df)} tv to {output_file}")
