...
- **2026-05-17 (session 3):** Resolved off-screen `ViewShot` capture bug on Android (which previously captured a cropped portion of the visible bottom navigation bar instead of the `ShareCard`). Wrapped the `ViewShot` in a zero-size, hidden overflow container (`width: 0, height: 0, overflow: 'hidden'`) to ensure the Android hardware renderer allocates a perfect drawing cache for the capture while remaining completely invisible on-screen.
